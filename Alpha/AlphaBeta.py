...
class AlphaBetaPlayer:
    def __init__(self, symbol, game: DotsAndBox, max_depth=3,method_bot = None):
        self.symbol = symbol
        self.game = game
        self.max_depth = max_depth
        # self.select_meth_bot = ResnetBOT(self.game.input_m,self.game.input_n,self.game,args_Res)
        self.select_meth_bot = Greedy_Bot_2(self.game)
        if method_bot:
    ...
    def alphabeta(self, game, depth, alpha, beta, maximizing):
        ...
        if maximizing:
            ...
            # random.shuffle(valid_moves)  # 打亂順序
            # for move in valid_moves[:min(self.max_expand, len(valid_moves))]:
